	...
	const api = {
		...
		async register(
			...
			try {
				...
				if (!sessionToken && !emailVerified && member.status === 'ACTIVE') {
					...
					return experiments['specs.thunderbolt.HangingSignupForNeededApprovalFlow']
						? hangingPromise()
						: { member, status }
		...
		async showCustomAuthenticationDialog(
			...
			return new Promise(async (resolve, reject) => {
				...
				smPopups.assignRequestAuthenticationResolveInstance(resolve)
				smPopups.assignRequestAuthenticationRejection(reject)
				const cbid = api.registerToUserLogin(async () => {
					smPopups.requestAuthenticationResolveInstance({
						member: memberDetails,
