...
const isExternalLinkNonPremiumDomain = (url: string) => {
...
export const resolveDocumentLink = (
	...
	isDocumentForPremiumDomainsExperimentOn: boolean,
	isExcludeOriginForAllPremiumDomainExperimentOn: boolean
): string => {
	const conditionToChangeURL =
		docId.endsWith('pdf') &&
		isDocumentForPremiumDomainsExperimentOn &&
		isPremiumDomain &&
		isExternalLinkNonPremiumDomain(externalBaseUrl)

	const baseUrl = conditionToChangeURL
		? isExcludeOriginForAllPremiumDomainExperimentOn
			? new URL('_files', new URL(externalBaseUrl))
			: new URL('_files', new URL(externalBaseUrl).origin)
