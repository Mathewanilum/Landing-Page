  ...
  handleSubmit = async (event) => {
    ...
    try {
      ...
      if (this.props.isActive) {
        const isUseNewPcfServiceEnabled = this.props.experiments.enabled(
          'specs.chat.usePcfService',
        );
        const typeValueFields = fields
          .map(({ type, value }) => ({
            type: isUseNewPcfServiceEnabled ? type.toUpperCase() : type,
            value: value.trim() || undefined,
          }))
          .filter(({ value }) => !!value);
        if (isUseNewPcfServiceEnabled) {
          await this.props.serverApi.postPCF(typeValueFields);
        } else {
