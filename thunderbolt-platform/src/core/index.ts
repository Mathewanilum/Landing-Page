...
export function createPlatformAPI() {
	...
	return {
		...
		async runPlatformOnPage({ bootstrapData, logger, importScripts, moduleLoader, viewerAPI, fetchModels, sessionService, debugApi, flushPendingUpdates = _.noop }: InitArgs) {
			...
			if (!isSSR) {
				handlers.componentsLoader.registerOnPropsChangedHandler(bootstrapData.currentContextId, (changes: CompProps) => {
			...
			const DocumentSdkLoader = async () => Promise.resolve(DocumentSdkFactory({ modelsApi, wixSelector, currentPageId: bootstrapData.currentPageId }))
