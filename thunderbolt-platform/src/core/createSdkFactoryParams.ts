...
import type { PlatformUtils, SdkInstance, PlatformLogger, Connection, $W, PlatformEnvData, $WScope, EventContext, ModelsAPI, WixCodeNamespacesRegistry } from '@wix/thunderbolt-symbols'
		...
		getSdkFactoryParams: ({ compId, connection, compType, controllerCompId, role, getInstance, create$w, $wScope }) => {
			...
			return {
				...
				platformUtils: {
					...
					wixCodeNamespacesRegistry: {
						get: (namespace) => wixCodeNamespacesRegistry.get(namespace),
					},
				},
				...platformUtils,
				styleUtils,
